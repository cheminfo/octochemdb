import { readFileSync } from 'fs';

import Debug from 'debug';
import { fileListFromZip } from 'filelist-from';

import { ncbiTaxonomyParser } from './ncbiTaxonomyParser.js';

const debug = Debug('importOneTaxonomyFile');

export default async function importOneTaxonomyFile(
  connection,
  progress,
  file,
  options,
) {
  const collection = await connection.getCollection('taxonomies');

  debug(`Importing: ${file.name}`);
  // should we directly import the data how wait that we reach the previously imported information
  let { shouldImport = true, lastDocument } = options;

  const fileList = (await fileListFromZip(readFileSync(file.path))).filter(
    (file) => file.name === 'fullnamelineage.dmp',
  );
  const arrayBuffer = await fileList[0].arrayBuffer();

  let counter = 0;
  let imported = 0;
  for await (const taxonomy of ncbiTaxonomyParser(arrayBuffer)) {
    if (process.env.TEST === 'true' && counter++ > 20) break;
    if (!shouldImport) {
      if (taxonomy.id !== lastDocument._id) {
        continue;
      }
      debug(`Skipping taxonomies till: ${lastDocument._id}`);
      continue;
    }
    shouldImport = true;
    taxonomy._seq = ++progress.seq;
    taxonomy._source = file.path.replace(process.env.ORIGINAL_DATA_PATH, '');
    await collection.updateOne(
      { _id: taxonomy.id },
      { $set: taxonomy },
      { upsert: true },
    );
    connection.setProgress(progress);
    imported++;
  }

  debug(`${imported} taxonomies processed`);
}
