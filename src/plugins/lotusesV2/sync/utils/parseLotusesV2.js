...
const DATA_DIR = 'data/originalData/lotusesV2';

/**
 * Downloads a SPARQL query result as TSV and streams it to a local file.
 * @param query - The SPARQL query.
 * @param filePath - Where to write the TSV file.
 * @param [retries=3] - Number of retries on failure.
 * @returns
 */
async function downloadSparqlToFile(query, filePath, retries = 3) {
  for (let attempt = 0; attempt < retries; attempt++) {
    try {
      ...
      debug.warn(
        `  Attempt ${attempt + 1}/${retries} failed: ${error.message}`,
      );
...
function getIdFromIRI(iri) {
  return /** @type {string} */ (iri.replaceAll(/^<|>$/g, '').split('/').pop());
}
 ...
 * @param taxaMap - The taxa map to populate.
 * @returns
 */
...
`;
      try {
        ...
        debug.warn(`  Error fetching parent taxa chunk: ${error.message}`);
      }
    }
  }
}
 ...
 * @param taxonId - The starting taxon Wikidata ID.
 * @param taxaMap - The complete taxa map with parent links.
 * @returns An object with kingdom, phylum, class, order, family, genus, species fields.
 */
 ...
 * @param testData - Pre-built test data mimicking SPARQL bindings.
 * @param connection - Active database connection wrapper.
 * @yields {LotusV2Entry}
 * @returns
 */
