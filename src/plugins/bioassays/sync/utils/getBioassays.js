 ...
 * @param bioassaysFilePath - Absolute path to the gzipped TSV bioassay dump file.
 ...
 * @returns Resolves with a map of AID → bioassay entry, or `undefined` if a fatal error occurred.
 */
    ...
    for await (let line of lines) {
      ...
      if (taxonomyIDs) {
        if (taxonomyIDs.includes('|')) {
          for (const entry of taxonomyIDs.split('|')) {
      ...
      if (targetTaxIDs) {
        if (targetTaxIDs.includes('|')) {
          for (const entry of targetTaxIDs.split('|')) {
            targetTaxonomies[entry] = [];
          }
        } else {
