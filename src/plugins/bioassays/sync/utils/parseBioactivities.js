 ...
 * @param bioActivitiesFilePath - Absolute path to the gzipped TSV bioactivities dump file.
 * @param bioassaysFilePath - Absolute path to the gzipped TSV bioassays dump file.
 * @param connection - Active OctoChemDB connection used for error reporting. Errors are persisted to the admin collection and monitored externally; no exception is re-thrown.
 * @param collectionCompounds - MongoDB collection storing compound documents (keyed by numeric CID).
 * @param collectionTaxonomies - MongoDB collection storing taxonomy documents (keyed by numeric taxon ID).
 * @param oldToNewTaxIDs - Map of deprecated taxonomy IDs (keys) to their current replacement IDs (values).
...
async function* parseBioactivities(
  ...
  try {
    ...
    for await (let line of lines) {
      ...
      if (connection && process.env.NODE_ENV === 'test' && counter > 50) break;
    }
  } catch (error) {
