...
export default async function getNpassesLastFiles(connection) {
  ...
  try {
    ...
    if (process.env.NODE_ENV === 'test') {
      lastFile = `src/plugins/npasses/sync/utils/__tests__/data/generalTest.txt`;
      lastFileActivity = `src/plugins/npasses/sync/utils/__tests__/data/activitiesTest.txt`;
      lastFileSpeciesProperties = `src/plugins/npasses/sync/utils/__tests__/data/propertiesTest.txt`;
      lastFileSpeciesInfo = `src/plugins/npasses/sync/utils/__tests__/data/speciesInfoTest.txt`;
      lastFileSpeciesPair = `src/plugins/npasses/sync/utils/__tests__/data/speciesPairTest.txt`;
      lastTargetInfo = `src/plugins/npasses/sync/utils/__tests__/data/targetInfoTest.txt`;
      ...
      const options = {
        ...
        destinationLocal: `data/originalData/npasses/full`,
      ...
      sources = [
        lastFile.replace(`data/originalData/`, ''),
        lastFileActivity.replace(`data/originalData/`, ''),
        lastFileSpeciesProperties.replace(`data/originalData/`, ''),
        lastFileSpeciesInfo.replace(`data/originalData/`, ''),
        lastFileSpeciesPair.replace(`data/originalData/`, ''),
        lastTargetInfo.replace(`data/originalData/`, ''),
      ];
    }
