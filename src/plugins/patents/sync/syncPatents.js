 ...
 * @returns Resolves when synchronisation is complete or when
 *   no update is needed.
 */
export async function sync(connection) {
  ...
  try {
    ...
    let options = {
      destinationLocal: `data/originalData//patents/`,
    ...
    if (process.env.NODE_ENV === 'test') {
      ...
      titles2parse = [
        'src/plugins/patents/sync/utils/__tests__/data/pc_patent2title.ttl.gz',
      ];
      abstracts2parse = [
        'src/plugins/patents/sync/utils/__tests__/data/pc_patetn2abstract.ttl.gz',
      ];
