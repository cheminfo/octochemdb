 ...
 * @returns returns collection lccs
 */
export async function sync(connection) {
  ...
  let options = {
    ...
    destinationLocal: `data/originalData/lccs/full`,
    collectionName: 'lccs',
    filenameNew: 'lccs',
    extensionNew: 'gz',
  };
  try {
    ...
    if (process.env.NODE_ENV === 'test') {
      lastFile = `src/plugins/lccs/sync/utils/__tests__/data/test2.xml.gz`;
      sources = [lastFile];
    } else {
    ...
    if (isTimeToUpdate) {
      ...
      let fileGHS;
      if (process.env.NODE_ENV === 'test') {
        fileGHS = 'src/plugins/lccs/sync/utils/__tests__/data/ghs.txt';
      } else {
