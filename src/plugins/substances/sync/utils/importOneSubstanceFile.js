 ...
 * @param file - substance SDF file descriptor
 * @param options - import options (`shouldImport`, `lastDocument`)
 * @returns number of substances imported
 */
    ...
    for await (const chunk of unzipStream) {
      ...
      if (lastIndex > 0 && lastIndex < bufferValue.length - 5) {
        ...
        newSubstances += await parseSDF(
          bufferValue.slice(0, Math.max(0, lastIndex + 5)),
        );
        bufferValue = bufferValue.slice(Math.max(0, lastIndex + 5));
      }
    }
