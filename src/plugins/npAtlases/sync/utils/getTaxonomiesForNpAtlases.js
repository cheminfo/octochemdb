 ...
 * @param entry - The parsed NPAtlas entry containing raw
 *   taxonomy data in `entry.data.taxonomies[0]`.
 * @param taxonomiesCollection - MongoDB collection handle for
 *   the `taxonomies` collection.
 * @param oldToNewTaxIDs - Deprecated → current
 *   taxonomy-ID mapping from `taxonomySynonyms()`.
 * @returns Array of standardised taxonomy objects (usually
 *   a single element).
 */
