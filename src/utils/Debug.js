 ...
 * @function Debug
 * @param context - The context or namespace for the logger.
 * @returns An object containing logging methods for different levels (fatal, error, warn, info, debug, trace).
 */
    ...
    logger[level] = async (message, options = {}) => {
      ...
      if (options && Object.keys(options).length > 0) {
        ...
        try {
          ...
          throw new Error(`Error logging message: ${error}`, { cause: error });
        }
      }
    };
  }
 ...
 * @param messages - Array of message objects to be sent via Telegram.
 ...
 * @returns Promise that resolves when the log is stored in the database.
 */
