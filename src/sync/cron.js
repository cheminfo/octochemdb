...
async function cron() {
  ...
  if (process.env.PLUGINS) {
    const allowedPlugins = new Set(process.env.PLUGINS.split(','));
    syncURLs = syncURLs.filter((url) => {
      ...
      if (allowedPlugins.has(pluginName)) return true;
      return false;
    });
  }
  if (process.env.EXCLUDEPLUGINS) {
    const notAllowedPlugins = new Set(process.env.EXCLUDEPLUGINS.split(','));
    syncURLs = syncURLs.filter((url) => {
      const pluginName = url.pathname.replace(
        /.*plugins\/\/?(?<temp2>.*?)\/.*/,
        '$1',
      );
      if (!notAllowedPlugins.has(pluginName)) return true;
      return false;
    });
  }
