    ...
    .map((line) => {
      ...
      if (groups.size > 0) {
        if (groups.size.endsWith('K')) {
          groups.size = Number.parseInt(groups.size, 10) * 1024;
        } else if (groups.size.endsWith('M')) {
          groups.size = Number.parseInt(groups.size, 10) * 1024 * 1024;
        } else if (groups.size.endsWith('G')) {
          groups.size = Number.parseInt(groups.size, 10) * 1024 * 1024 * 1024;
        } else {
          groups.size = Number.parseInt(groups.size, 10);
        }
      }
