...
(function () {
  ...
  if (typeof knownDatabases !== 'undefined') {
  ...
  if (typeof collection === 'undefined') {
    throw (
      "You have to supply a 'collection' variable, à la --eval 'var collection = \"animals\"'.\n" +
  ...
  var readConfig = function (configProvider) {
    ...
    var read = function (name, defaultValue) {
      var value =
        typeof configProvider[name] !== 'undefined'
          ? configProvider[name]
          : defaultValue;
      config[name] = value;
  ...
  var serializeDoc = function (doc, maxDepth, excludeSubkeys) {
    ...
    var arrayRegex = new RegExp(
      '\\.' + config.arrayEscape + '\\d+' + config.arrayEscape + '\\.',
      'g',
    );
  ...
  var reduceDocuments = function (accumulator, object) {
    var docResult = analyseDocument(
      serializeDoc(object, config.maxDepth, config.excludeSubkeys),
    );
  ...
  var createAsciiTable = function (results) {
    ...
    var colMaxWidth = function (arr, index) {
      return Math.max.apply(
        ...
        arr.map(function (row) {
          return row[index] ? row[index].toString().length : 0;
        }),
      );
    };
    var pad = function (width, string, symbol) {
      return width <= string.length
        ? string
        : pad(width, isNaN(string) ? string + symbol : symbol + string, symbol);
    };
